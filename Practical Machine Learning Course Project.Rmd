...
An overall pseudo-random number generator seed was set at 4321 for all code. In order to reproduce the results below, the same seed should be used.
...
#download training.csv and testing.csv 
...
dim(trainingset)
dim(testingset)
# in the interest of saving space will not display content of the datasets head commands
# head(trainingset)
# head(testingset)
```
...
plot(subTraining$classe, col="gray", pch = .5, main="Bar Plot of levels of the variable classe using subTraining data set", xlab="classe levels", ylab="Frequency")
```
